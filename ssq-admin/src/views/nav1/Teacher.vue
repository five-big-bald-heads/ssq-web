<template>
	<section>
		...
		<el-table :data="users" highlight-current-row ref = "multipleTable" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="stno" sortable label="工号" >
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="工号" prop="stno" >
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				this.$axios.get('/api//Teacher/Page',{params:{indexpage:para.indexpage,everypage:para.everypage}})
			...
			findUser() {
				...
				this.listLoading = true;  //获取数据成功
				var second_this = this;
				this.$axios.get('/api/User?',{params:{name:para.name}})
				.then(function (response) {
					//console.log(response.data);
